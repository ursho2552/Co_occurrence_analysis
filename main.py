...
def main():
    '''
    This is the main function used to construct the score matrices and analyze them
    '''
    ...
    #baseline/future projection, algorithm used (GAM/GLM/RF/ANN), climate model used,
    # and predictor pool
    ...
    #=====================================================================
    #Now that we have a unique code for each file in list_files, we can easily
    #identify each future projection and the corresponding baseline
    ...
    for my_list in list_of_values:

        my_arguments = (list_files,file_names,my_list,clusters,all_models, my_config)
        parallel_job = multiprocessing.Process(target=functions.calculate_scores, args=my_arguments)

        jobs.append(parallel_job)
        parallel_job.start()
...
#==============================================================================
    ...
    my_list = functions.find_baseline(file_names, models=models,
                                    algorithm_names=algo_names, predictors=params,
                                    clusters=clusters, configurations=my_config)
    ...
    for i in tqdm(my_list, desc='Calculating statistics'):
        ...
        logging.warning('All')
        loss_a,gain_a,const_a,_ = functions.get_difference(matrix_baseline, matrix_projection)
        tot_a = loss_a + gain_a + const_a
        logging.warning(np.array([loss_a,gain_a,const_a])/tot_a)

        logging.warning('phyto:')
        loss_p,gain_p,const_p,_ = functions.get_difference(matrix_baseline[:338,:338], matrix_projection[:338,:338])
        tot_p = loss_p + gain_p + const_p
        logging.warning(np.array([loss_p,gain_p,const_p])/tot_p)

        logging.warning('zoo:')
        loss_z,gain_z,const_z,_ = functions.get_difference(matrix_baseline[338:,338:], matrix_projection[338:,338:])
        tot_z = loss_z + gain_z + const_z
        logging.warning(np.array([loss_z,gain_z,const_z])/tot_z)

        logging.warning('phyto-zoo:')
        loss_pz,gain_pz,const_pz,_ = functions.get_difference(matrix_baseline[:338,338:], matrix_projection[:338,338:])
        tot_pz = loss_pz + gain_pz + const_pz
        logging.warning(np.array([loss_pz,gain_pz,const_pz])/tot_pz)

        all_results[counter,:] = [loss_a/tot_a,loss_p/tot_p,loss_z/tot_z,loss_pz/tot_pz,tot_a,tot_p,tot_z,tot_pz]
        counter += 1
        all_results[counter,:] = [gain_a/tot_a,gain_p/tot_p,gain_z/tot_z,gain_pz/tot_pz,tot_a,tot_p,tot_z,tot_pz]
        counter += 1
        all_results[counter,:] = [const_a/tot_a,const_p/tot_p,const_z/tot_z,const_pz/tot_pz,tot_a,tot_p,tot_z,tot_pz]
        counter += 1
    ...
    if clusters is not None:
        filename_results = f'{my_config.directory_output}Cluster_All_results.csv'
        filename_results_description = f'{my_config.directory_output}Cluster_All_results_description.csv'
    else:
        filename_results = f'{my_config.directory_output}All_results.csv'
        filename_results_description = f'{my_config.directory_output}All_results_description.csv'

    np.savetxt(filename_results, all_results, delimiter=',')

    with open(filename_results_description,"w+", encoding="utf-8") as my_csv:
        csv_writer = csv.writer(my_csv,delimiter=',')
        csv_writer.writerows(all_results_text)
